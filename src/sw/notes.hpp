...
template<std::floating_point F = double>
constexpr F defaultStandardPitch = static_cast<F>(440);
...
template<std::floating_point F>
Note toNote(const F frequency, const F standardPitch = defaultStandardPitch<F>, F *o_semitoneDeviation = nullptr)
{
    const auto safeAssign = [](const F &from, F *o_to) {
        if (o_to != nullptr)
            *o_to = from;
    };
    ...
    {
        safeAssign(math::zero<F>, o_semitoneDeviation);
    ...
    safeAssign(diffSemitones - roundDiffSemitones, o_semitoneDeviation);
    return fromMidi(69 + static_cast<int>(roundDiffSemitones));
}

template<std::floating_point F>
F toFrequency(const Note &note, const F standardPitch = defaultStandardPitch<F>,
              const F semitoneDeviation = math::zero<F>)
{
