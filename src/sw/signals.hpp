                                                     ...
                                                     const unsigned int seed = 0u)
{
    ...
    return std::ranges::to<std::vector>(
      std::views::iota(0, static_cast<int>(length)) | std::views::transform([&](const auto) {
...
std::vector<F> makeRandomRealSignal(const F amplitude, const size_t length, const unsigned int seed)
{
    ...
    return std::ranges::to<std::vector>(std::views::iota(0, static_cast<int>(length)) |
...
std::vector<F> makeSineWave(const F amplitude, const F frequency, const F sampleRate, const size_t length)
{
    ...
    return std::ranges::to<std::vector>(std::views::counted(infiniteSignal.begin(), static_cast<int>(length)));
}
...
{
    return hammingWindow<F>(length) | std::ranges::to<std::vector>;
}

template<std::floating_point F>
std::vector<F> makeVonHannWindow(size_t length)
{
    return std::ranges::to<std::vector>(vonHannWindow<F>(length));
}
