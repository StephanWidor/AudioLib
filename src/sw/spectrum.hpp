...
template<std::floating_point F, ranges::TypedInputRange<SpectrumValue<F>> Spectrum>
    requires std::ranges::sized_range<Spectrum>
SpectrumValue<F> findFundamental(Spectrum &&spectrum, const F squaredGainsThreshold,
                                 const F maxFrequency = static_cast<F>(5000))
{
