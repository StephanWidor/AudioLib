...
TYPED_TEST(SpectrumTest, toSpectrumByPhase)
{
    ...
    const auto check = [&](auto &&frequencies, auto &&phases, auto &&coefficients, auto &&spectrum) {
        ...
        phases.clear();
        std::ranges::transform(frequencies, std::back_inserter(phases), [&](const auto frequency) {
            return math::angles::standardized(-phaseAngle(frequency, timeDiff));
        });
        ...
        // quite arbitrary tolerance, but well...
        EXPECT_TRUE(equal<F>(frequencies, sw::frequencies<F>(spectrum), static_cast<F>(4) * math::defaultTolerance<F>));
    };
